...
트랜잭션을 이용해서 일련의 작업을 완벽하게 처리하지 못했을때 원 상태로 복구해서 일부만 적용되는 상황을 방지할 수 있습니다.`,
  },
  {
    ...
    content: `트랜잭션은 ACID라는 4가지 특성을 만족해야합니다.
...
Non-Repeatable Read의 한 종류로 조회해온 결과의 행이 새로 생기거나 없어지는 현상이다.`,
  ...
  },
  {
    ...
    content: `데이터베이스에서 데드락이란 두 개 이상의 트랜잭션이 서로 원하는 데이터의 Lock 풀리길 기다리는 걸 의미합니다. 이는 시스템 리소스를 잡아먹어 해결을 해주어야 하는데
...
따라서 보통의 경우 회피 기법을 주로 사용하며 트랜잭션의 시작 순서에 따라 뒤늦게 들어온 트랜잭션이 롤백하여 자원을 양보하는 방식입니다.`,
  },
  {
    ...
    content: `저장 프로시저는 데이터베이스 내에 저장되는 일련의 SQL문의 집합으로 단순히 저장 프로시저를 호출함으로써 쿼리의 결과를 받아오기 때문에 반복적으로 수행하거나 복잡한 쿼리의 SQL문을 단순화하는데 사용하는 것으로 알고 있습니다.
`,
  },
  {
    title: "Statement와 PrepareStatement에 대해 설명해주세요.",
    content: `Statement는 SQL 쿼리를 실행하기 위한 인터페이스로 매번 SQL 문을 실행할때마다 새로운 컴파일을 하지만 PreparedStatement는 컴파일된 쿼리를 재사용하고 동적 쿼리와 SQL 인젝션 같은 보안에 더 좋다고 알고 있습니다.
`,
  },
  { title: "소프트 딜리트에 대해 설명해주세요.", content: `설명없음` },
  {
    title: "ORM과 ORM 사용시 장단점을 설명해주세요.",
    content: `ORM은 객체와 관계형 데이터베이스의 데이터를 매핑하는 것으로
...
`,
  },
  {
    ...
    content: `RDBMS는 관계형 데이터 베이스로 정해진 스키마에 따라 데이터가 저장되고 관계를 통해 여러 테이블로 데이터가 분산되어 저장됩니다.
...
`,
  ...
  { title: "아이솔레이션 레벨에 대해 설명해주세요.", content: `설명없음` },
];
